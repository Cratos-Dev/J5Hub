...
local G_lastKnownHealth      = (player.Character and player.Character:FindFirstChildOfClass("Humanoid") and player.Character:FindFirstChildOfClass("Humanoid").Health) or 100
...
loadSettings()
-- Autosave every 15 seconds (reduced from 5 to lower background CPU usage)
task.spawn(function()
    while task.wait(15) do saveSettings() end
end)
...
RunService.Heartbeat:Connect(function()
    if not infJumpEnabled then return end
    _ijTick=_ijTick+1 if _ijTick%2~=0 then return end
    local h=getHRP() if not h then return end
...
local function runTPDown()
    task.spawn(function()
        ...
        if hit then
            ...
            -- hrp.CFrame direct write removed (PivotTo above handles it safely)
...
local function startBatAimbot()
    ...
    -- (no physics constraints created — nothing to clean up)

    -- Facing handled via CFrame.lookAt directly (no AlignOrientation — server detectable)
...
local function stopBatAimbot()
    ...
    task.spawn(function()
        for _ = 1, 3 do
            ...
            if hm2 then hm2.AutoRotate = true end  -- PlatformStand write removed (detectable in loop)
            if hrp2 then
                pcall(function()
...
local function startAutoPlayLeft()
    ...
    aplConn=RunService.Heartbeat:Connect(function()
        ...
        elseif aplPhase==0 then
            -- Safety: advance to phase 3 after 0.15s (gives physics time to settle after wall)
            if tick()-_aplPhase0Start>=0.15 then aplPhase=3 end
            return
...
local function startAutoPlayRight()
    ...
    aprConn=RunService.Heartbeat:Connect(function()
        ...
        elseif aprPhase==0 then
            -- Safety: advance to phase 3 after 0.15s (gives physics time to settle after wall)
            if tick()-_aprPhase0Start>=0.15 then aprPhase=3 end
            return
...
local function startAutoSteal()
    ...
    autoStealConn = RunService.Heartbeat:Connect(function()
        ...
        if p then
            -- zero velocity only once when starting a steal, not every frame
...
local function stopAutoSteal()
    ...
    _plotCache={} _plotCacheTime={} _cachedPrompts={} _stealData={}
    ...
    task.spawn(function()
        pcall(function()
            ...
            local function tp(pos)
                ...
                local cf=CFrame.new(pos+Vector3.new(0,3,0))*CFrame.Angles(0,rotation,0)
...
RunService.Heartbeat:Connect(function()
    ...
    -- Only correct WalkSpeed if it drifted by more than 2 (avoids constant writes)
    if math.abs(gHum.WalkSpeed - targetSpd) > 2 then gHum.WalkSpeed = targetSpd end
...
RunService.Heartbeat:Connect(function()
    ...
    if math.abs(gHum.WalkSpeed - SLOW_SPEED) > 2 and gHum.WalkSpeed > 0 then
        gHum.WalkSpeed = SLOW_SPEED
    end
end)
...
do
    ...
    fAuto.Size = UDim2.new(0, 120, 0, 50)
...
local function createToggle(parent,labelText,enabledKey,defaultOn,order,onToggle)
    ...
    row.Size=UDim2.new(0.96,0,0,38) row.BackgroundColor3=C_CARD
    ...
    label.TextColor3=Color3.new(1,1,1) label.Font=Enum.Font.GothamBold label.TextSize=13
    ...
    kbBtn.TextSize=8 kbBtn.ZIndex=10 kbBtn.AutoButtonColor=false
    ...
    if ctrlName then
        ...
        cPill.Font=Enum.Font.GothamBold cPill.TextSize=8 cPill.ZIndex=10 cPill.AutoButtonColor=false
...
local function createSection(parent,text,order)
    ...
    lbl.TextColor3=Color3.fromRGB(130,130,130) lbl.Font=Enum.Font.GothamBold lbl.TextSize=11
...
-- Bigger GUI: fixed 310x560 base, UIScale handles device fit via Roblox's built-in screen scaling
local BASE_W,BASE_H=310,560
...
headerTitle.Font=Enum.Font.GothamBlack headerTitle.TextSize=26
...
headerDiscord.Font=Enum.Font.GothamBold headerDiscord.TextSize=11
headerDiscord.TextXAlignment=Enum.TextXAlignment.Center headerDiscord.ZIndex=4
do
    local t=0 local dir=1 local _tc=0
    RunService.Heartbeat:Connect(function(dt)
        if not headerTitle or not headerTitle.Parent then return end
        _tc=_tc+1 if _tc%3~=0 then return end  -- throttle: update every 3 frames only
        t=t+dt*0.7*dir
...
local function createTab(tabName,order)
    ...
    tabBtn.Font=Enum.Font.GothamBold tabBtn.TextSize=11 tabBtn.LayoutOrder=order tabBtn.ZIndex=5
...
usernameLabel.TextSize=14 usernameLabel.TextXAlignment=Enum.TextXAlignment.Left
...
-- ═══════════════════════════════════════════════════════════════
local MB_BW=120 local MB_BH=50 local MB_GAP=8
...
local function createMobileButton(text,row,col,callback,isMomentary,bindKey,isPersistent)
    ...
    label.Font=Enum.Font.GothamBlack label.TextSize=13 label.ZIndex=102
...
local dsPanel = Instance.new("Frame", gui)
...
local function closeDesync()
    TweenService:Create(dsPanel, TweenInfo.new(0.2, Enum.EasingStyle.Quart, Enum.EasingDirection.Out),
        {BackgroundTransparency=1}):Play()
    task.delay(0.21, function()
        if dsPanel and dsPanel.Parent then dsPanel.Visible=false end
    end)
end
...
UserInputService.InputBegan:Connect(function(input, gpe)
    ...
    elseif kc == Keybinds.AutoLeft then
        ...
        if on then
            ...
            if aprOn then stopAutoPlayRight() if VisualSetters.AutoRight then VisualSetters.AutoRight(false) end end
            aplOn=true startAutoPlayLeft()
        else stopAutoPlayLeft(); aplOn=false end
        if VisualSetters.AutoLeft then VisualSetters.AutoLeft(aplOn) end
        if mobBtnRefs["AutoLeft"] then TweenService:Create(mobBtnRefs["AutoLeft"],TweenInfo.new(0.15),{BackgroundColor3=aplOn and MB_ACTIVE or MB_BG}):Play() end
    elseif kc == Keybinds.AutoRight then
        ...
        if on then
            ...
            if aplOn then stopAutoPlayLeft() if VisualSetters.AutoLeft then VisualSetters.AutoLeft(false) end end
            aprOn=true startAutoPlayRight()
        else stopAutoPlayRight(); aprOn=false end
        if VisualSetters.AutoRight then VisualSetters.AutoRight(aprOn) end
        if mobBtnRefs["AutoRight"] then TweenService:Create(mobBtnRefs["AutoRight"],TweenInfo.new(0.15),{BackgroundColor3=aprOn and MB_ACTIVE or MB_BG}):Play() end
